...
use App\Traits\RoleCheckTrait; // 👈 ADD THIS

class UserController extends Controller
{
    use RoleCheckTrait; // 👈 ADD THIS

    public function index(Request $request)
    {
        // Check permission - only admin and manager can view users list
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            // Log activity
            auth()->user()->logActivity('create', 'users', 'Created user: ' . $user->name);
    ...
    {
        // Check permission - only admin and manager can view user details
        $check = $this->checkRole(['admin', 'manager']);
        if ($check !== true) {
            return $check;
        }
    ...
    {
        // Check permission - only admin and manager can edit users
        $check = $this->checkRole(['admin', 'manager']);
        if ($check !== true) {
            return $check;
        }
    ...
    {
        // Check permission - only admin and manager can update users
        $check = $this->checkRole(['admin', 'manager']);
        if ($check !== true) {
            return $check;
        }
        ...
        try {
            ...
            // Log activity
            auth()->user()->logActivity('update', 'users', 'Updated user: ' . $user->name, $oldData, $user->toArray());
    ...
    public function destroy(User $user)
    {
        ...
        try {
            ...
            // Log activity
            auth()->user()->logActivity('delete', 'users', 'Deleted user: ' . $userName);
    ...
    {
        // Check permission - only admin can toggle user status
        $check = $this->checkRole(['admin']);
        if ($check !== true) {
            return $check;
        }
        ...
        try {
            ...
            // Log activity
            auth()->user()->logActivity('update', 'users', $statusText . ' user: ' . $user->name);
    ...
    {
        // Check permission - only admin and manager can view user activity
        $check = $this->checkRole(['admin', 'manager']);
        if ($check !== true) {
            return $check;
        }
    ...
    {
        // Check permission - only admin and manager can view all activity logs
        $check = $this->checkRole(['admin', 'manager']);
        if ($check !== true) {
            return $check;
        }
    ...
    {
        // Profile update is accessible to all logged-in users
        $user = auth()->user();
